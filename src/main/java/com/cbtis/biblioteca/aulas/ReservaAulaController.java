...
public class ReservaAulaController {
    ...
    public ResponseEntity<Object> reservarAula(@RequestBody ReservaAulaRequest reservaAula) {
        ...
        ReservaAula reserva = reservaAulaService.reservarAula(null, nombreProfesor, fecha, inicio, fin, grupo, materia);
        if (reserva != null) {
    ...
    public ResponseEntity<Object> actualizarReserva(@RequestBody ReservaAulaRequest request, @PathVariable Long reservaId){
        ReservaAula reserva = reservaAulaService.actualizarReserva(request,reservaId);
        if (reserva != null) {
            return ResponseEntity.ok(reserva);
        } else {
            String body = String.format("El aula no está disponible para la fecha %s y el intervalo de tiempo [%s, %s], el aula está disponible de 7:00 a 16:00 de lunes a viernes", request.getFechaReserva(), request.getHoraInicio(), request.getHoraFin());
            return ResponseEntity.status(HttpStatus.CONFLICT).body(body);
        }
    }
